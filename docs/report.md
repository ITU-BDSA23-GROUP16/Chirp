...
## Table of contents:
 ...
 - [Sequence of functionality/calls trough Chirp!](#sequence)
 - [Process](#Process)
...
There are two versions of our application. The main branch in our repository contains the version which is deployed on Azure. The chirp-sqlite branch contains the version released on Github. The Azure version uses SQLServer instead of SQLite for the database connection. SQLServer has stricter requirements for EFCore’s database model, which means that the Author class in the deployed version has two fields of lists of Authors, representing an Author’s Followed and Following authors.
...
We chose to switch back to SQLite because running an SQLite database doesn’t require any other programs to be installed and run, whereas SQLServer would require the user to start a Docker container or an SQLServer instance on their computer before starting Chirp, and they might do this in many different ways which would have to be accounted for in the connection string. 
...
We have chosen to do a UML Sequence Diagram for every call to any endpoint in our version of Chirp! The diagrams show the flow of data from the moment a user clicks around on the website, triggering GET or POST requests through Razor Pages, which are then handled through our defined methods for so in our relevant repositories. 
...
6: UML Sequence diagram of a user wanting to see all users in Chirp!
...
![](./images/UMLSequenceDiagram.drawio.png)

8: Complete UML Sequence diagram of the entire flow through time, as an unauthorized user goes to the root endpoint, registers, logs out, logs in, and goes through the features of Chirp! This diagram contains the same flows as the previous diagrams, but put together in one complete flow.
...
Our Chirp project has 3 GitHub action workflows, excluding the dependabot workflow.
...
The deployment workflow is triggered by pushes on the main branch and the “sqlserver” branch. Once again, the project is built sans test suite. An executable is made, but the used libraries are left external. An artefact (which is a file used by Azure to track and share dependencies) is uploaded first, then downloaded again and used as part of the deployment process.
...
2. Download the Chirp zip file depending on which operating system you are using.
...
7. If a warning shows up saying "“Chirp.Web” cannot be opened because the developer cannot be verified" follow the steps below bypass it.
    ...
    2. Go to "Sysstem Preferences" on your Mac
...
<a id="vscode"></a>
In **VsCode**:
1. To clone our project, you start by navigating towards our public GitHub with the name: ITU-BDSA23-GROUP16. There should be a public repository named ‘Chirp’ when clicking on ‘Repositories’ in the top left corner. 
2. With Git installed on the computer, the repository can be cloned by opening a terminal and then typing in the command ‘git clone’ followed by the link to the repository found when clicking on the ‘< > Code’ button in the repository.
You can choose where to clone the repository in the terminal before running the command, it will create a new folder named “Chirp” in that destination
3. After the cloning process has been done, you should be able to open the folder in VSCode and view the code with no additional setup.
Open a terminal inside our project and choose a project to run by typing the following into the command line: dotnet run --project src/Chirp.Web.
4. The program should then be able to run and you should be able to follow a link to a localhost instance of the web application through the terminal.
5. The first thing you should be able to see in our program is the ‘Public timeline’ with all the cheeps displayed from our database. You can’t do much before you’re signed into our system, so to do that, you’ll have to navigate towards the ‘Register’ page in the navigation bar and then register with any mail address you want. Once your registration has been completed, you should then head towards the ‘log in’ page that is also located in the navbar and log in with the same mail you’ve registered with. Once that’s done, you’re free to explore our Chirp webapp. 
